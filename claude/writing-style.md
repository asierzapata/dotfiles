...
Rules for writing prose intended for engineering readers: architectural docs, RFCs, explainers, PR descriptions, design proposals, and similar. These merge principles from William Zinsser's *On Writing Well* with patterns established across the documentation I have written.

The goal of this writing is to **teach** and, when needed, **convince**. Not to entertain.

This file is the shared source. Skills and commands that produce prose should reference `~/.claude/writing-style.md` rather than holding their own copy.

## Core Principles

- **Clear thinking becomes clear writing:** one cannot exist without the other. Before writing a section, ask: what am I trying to say?
...
- **Conversational but professional:** approachable, not casual. The reader should feel they are being walked through something by someone who knows it well, not reading a spec document.
...
- **Convicted when recommending:** when proposing a solution, be clear about why you believe it is the right one. Label recommendations explicitly (e.g. "Option A: ... (Recommended)").
...
- **Don't use em-dashes for asides.** Use commas, parentheses, colons, or split the sentence. Em-dashes used this way are a strong AI tell.
...
## Avoid AI-Sounding Cadence

These patterns make prose feel machine-generated even when the ideas are correct. After drafting any paragraph, scan for them explicitly.

- **Rhythmic antithesis with dropped verbs:** "The shapes are twins. The system support is not." reads as profound but is the single most common LLM cadence. Write the second clause out, or merge into one sentence: "The shapes are twins, but the framework doesn't treat them the same way."
- **Universal appeals to feeling:** "every tenured engineer has felt", "we all know", "anyone who has worked on X". Replace with a concrete moment: "We shipped the third near-duplicate of this handler last quarter in dispatch.ts."
- **Aphoristic closers:** sentences that try to land a profound-sounding line at the end of a paragraph ("This layer puts a name on a discomfort..."). Cut them, or replace with the concrete next step.
- **Uniform short sentences:** four or five clipped sentences in a row read as AI even when each is fine in isolation. Force at least one sentence per paragraph to carry a subclause or a real example, not just rhythm.
- **Three-beat parallelism:** "X does A. Y does B. Z does C." Vary connectors, merge two of the items, or break the pattern with an aside.
- **Vague abstract nouns without an anchor:** "a discomfort", "a tension", "a friction", "a pattern we keep seeing". Name the file, the bug, the PR number, or delete the sentence.
- **"Puts a name on" / "names a thing" framing:** models reach for this when summarizing. If the section already showed the thing, don't congratulate the reader for noticing it.
- **Em-dashes as asides:** see the formatting rule above. Use commas, parentheses, or a period.
